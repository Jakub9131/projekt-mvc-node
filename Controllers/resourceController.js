...
// Statyczna baza dostępnych kurierów i województw
    ...
    try {
        ...
        if (req.session.user.role === 'admin') {
            ...
            if (search && search.trim() !== '') {
                ...
                whereClause = {
                    [Op.or]: [
                        { resourceType: { [Op.iLike]: searchTxt } },
                        { city: { [Op.iLike]: searchTxt } },
                        { province: { [Op.iLike]: searchTxt } },
                        { status: { [Op.iLike]: searchTxt } },
                        { '$User.firstName$': { [Op.iLike]: searchTxt } },
                        { '$User.lastName$': { [Op.iLike]: searchTxt } },
                        { '$User.email$': { [Op.iLike]: searchTxt } }
                    ]
                };
            }

            rawOrders = await ResourceOrder.findAll({
                where: whereClause,
                include: [{
                    model: User,
                    required: false
        ...
        console.error('Błąd podczas ładowania zasobów z wyszukiwarką:', error);
